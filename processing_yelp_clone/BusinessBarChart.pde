  ...
  BusinessBarChart(int x, int y, Business[] businessChart, String label) {
    ...
    if (label.equals("topBusinesses")) {
      ...
      for (int i = 0; i < businessChart.length; i++) {
        bars[i] = new Bar(businessChart[i].getAverageStarsOfBusiness()*interval, tmpX, tmpY, businessChart[i].getBusinessName(), lineY2, z, "normal", 0);
      ...
      for (int i = 0; i < businessChart.length; i++) {
        bars[i] = new Bar(businessChart[i].returnAmountOfReviews()*interval, tmpX, tmpY, businessChart[i].getBusinessName(), lineY2, z, "normal", 0);
  ...
  BusinessBarChart(int x, int y, Review[] typeReviews, String label) {
    ...
    if (label.equals("funny")) {
      ...
      for (int i = 0; i < typeReviews.length; i++) {
        bars[i] = new Bar(typeReviews[i].getFunny()*interval, tmpX, tmpY, typeReviews[i].getAuthor(), lineY2, z, "normal", 0);
      ...
      lineY1 = y-max*interval;
      for (int i = 0; i < typeReviews.length; i++) {
        bars[i] = new Bar(typeReviews[i].getUseful()*interval, tmpX, tmpY, typeReviews[i].getAuthor(), lineY2, z, "normal", 0);
      ...
      lineY1 = y-max*interval;
      for (int i = 0; i < typeReviews.length; i++) {
        bars[i] = new Bar(typeReviews[i].getCool()*interval, tmpX, tmpY, typeReviews[i].getAuthor(), lineY2, z, "normal", 0);
