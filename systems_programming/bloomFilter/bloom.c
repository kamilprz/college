...
struct bloom * bloom_new(int size)
{
    ...
    result->bit_vector = bitset_new(size);
    return result;
}
...
{
    int result=0;
    result = bitset_lookup(this->bit_vector, atoi(item));
    return result;
}
...
{
    int hash;
    hash = hash_string(item, BLOOM_HASH1, this->bit_vector->sizeInBits);
    bitset_add(this->bit_vector,hash);
    hash = hash_string(item, BLOOM_HASH2, this->bit_vector->sizeInBits);
    bitset_add(this->bit_vector,hash);
}
